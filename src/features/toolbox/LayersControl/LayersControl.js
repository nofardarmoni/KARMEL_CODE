...
export default function LayersControl({ layers, setLayers }) {
  ...
  return (
    <div className={classes.root + " shadow"}>
      <span className={classes.title}>סדרן שכבות</span>
      {Object.keys(layers).map((layerKey) => (
        <Fragment key={layerKey}>
          {layers[layerKey].types ? (
            <>
              <FormControlLabel
                className={classes.treeCheckbox}
                control={
                  <StyledCheckbox
                    checked={getIsChecked(layerKey)}
                    onChange={(event) => handleChange(event)}
                    name={layerKey}
                    indeterminate={getIsIndeterminate(layerKey)}
                  />
                }
                label={`${layers[layerKey].label} (${getCheckedCount(
                  layerKey
                )})`}
              />
              <TreeView
              ...
              >
                <TreeItem className={classes.tree} nodeId={layerKey}>
                  <FormControl className={classes.subTrees}>
                    {Object.keys(layers[layerKey].types).map((typeKey) => (
                      <FormControlLabel
                        key={typeKey}
                        control={
                          <StyledCheckbox
                            checked={layers[layerKey].types[typeKey].isChecked}
                            onChange={(event) =>
                              handleCheckbox(event, layerKey)
                            }
                            name={typeKey}
                          />
                        }
                        label={layers[layerKey].types[typeKey].label}
                      />
                    ))}
                  </FormControl>
                </TreeItem>
            ...
            <FormControlLabel
              ...
              control={
                <StyledCheckbox
                  checked={layers[layerKey].isChecked}
                  onChange={(event) => handleChange(event)}
                  name={layerKey}
                />
              }

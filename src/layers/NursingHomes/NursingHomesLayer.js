...
function NursingHomesLayer({ polygon, distance }) {
  const classes = useStyles();
  const layerCache = useRef([]);

  const iconsUrl = "icons/layers/nursing-homes/";
  const icon = L.icon({
    iconSize: [25, 25],
    iconUrl: `${iconsUrl}nursing-homes.png`,
  });

  const { data } = useApiQuery({
    dataPath: "layers/nursinghomes",
...
export default deepCompareMemo(NursingHomesLayer);

...
function DialysisInstitutesLayer({ polygon, distance }) {
  const classes = useStyles();
  const layerCache = useRef([]);

  const iconsUrl = "icons/layers/dialysis-centers/";
  const icon = L.icon({
    iconSize: [25, 25],
    iconUrl: `${iconsUrl}dialysis-centers.png`,
  });

  const { data } = useApiQuery({
    dataPath: "layers/dialysisinstitutes",
    label: "מכוני דיאליזה",
    body: polygon && {
      polygon,
...
export default deepCompareMemo(DialysisInstitutesLayer);

...
function GeneralGraph({ }) {
    ...
    useEffect(() => {
        axios
            .get(
                `http://localhost:5000/earthquakeModule/general?magnitude=${newMagnitodeState}`
            )
    ...
    const graphData = Object.keys(generalData?.[0] ?? {}).flatMap((key) => {
        if (dictionairy[key]) return {
            rashut: dictionairy[key],
            count: generalData?.[0][key],
        }
        else return []
    }).sort((a, b) => b.count - a.count);
    // const graphData = Object.keys(generalData?.[0] ?? {})
    //     .map((key) => {
    //         if (dictionairy[key]) {
    //             return {
    //                 rashut: dictionairy[key],
    ...
    return (
        <GraphFrame
            hideGraph={graphData.length === 0 || newMagnitodeState < 4.5 || mode === "realtime"}

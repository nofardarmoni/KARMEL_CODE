...
function DistributionStationsGraph({ event }) {
    ...
    const totalCount = Math.ceil(useMemo(() => waterStationsData.reduce((acc, waterStation) => acc + parseInt(
        statecalc(
            mode,
    ...
    return (
        <GraphFrame
            ...
            title="כמות תחנות חלוקה"
            alternativeDesc={"אין נתונים על תחנות חלוקה"}
        >
            <div className={classes.averageTimeDesc}>
                ...
                <span>{" תחנות חלוקה"}</span>
            </div>
        </GraphFrame>
    );
}

export default deepCompareMemo(DistributionStationsGraph);
